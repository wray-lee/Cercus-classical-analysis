...
def plot_population_prewalk_integration(
    df: pd.DataFrame,
    figsize: tuple[float, float] = (8.5, 3.8),
) -> plt.Figure:
    """Population PreWalk integration diagnostic panel.

    Panel A: Per-subject response composition (Escape vs PreWalk stacked bar, ranked by total active rate).
    Panel B: Cross-subject coupling between PreWalk rate and Escape rate (scatter + linear regression).
    """
    trial_level = (
        df.groupby(["subject_id", "global_trial_index"])
    ...
    subject_stats = trial_level.groupby("subject_id").agg(
        total=("global_trial_index", "count"),
        escape=("response_type", lambda s: (s == "Escape").sum()),
        prewalk=("response_type", lambda s: (s == "PreWalk").sum()),
        no_resp=("response_type", lambda s: (s == "NoResponse").sum()),
    ).reset_index()

    subject_stats["escape_rate"] = subject_stats["escape"] / subject_stats["total"]
    subject_stats["prewalk_rate"] = subject_stats["prewalk"] / subject_stats["total"]
    subject_stats["no_resp_rate"] = subject_stats["no_resp"] / subject_stats["total"]
    subject_stats["resp_rate"] = subject_stats["escape_rate"] + subject_stats["prewalk_rate"]

    subject_stats = subject_stats.sort_values("resp_rate", ascending=False).reset_index(drop=True)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=figsize)

    # ── Panel A: Stacked bar per subject ──
    indices = np.arange(len(subject_stats))
    ax1.bar(
        indices,
        subject_stats["escape_rate"] * 100,
        color=COLOR_ESCAPE,
        alpha=0.85,
        label="Escape",
        width=0.7,
    ...
    ax1.bar(
        indices,
        subject_stats["prewalk_rate"] * 100,
        bottom=subject_stats["escape_rate"] * 100,
        color=COLOR_PREWALK,
        alpha=0.85,
        label="PreWalk",
        width=0.7,
    )
    ax1.set_xlabel("Subject (ranked by active response)", fontsize=9)
    ax1.set_ylabel("Response Rate (%)", fontsize=9)
    ax1.set_title("A. Cross-Animal Response Composition", fontweight="bold", fontsize=10)
    ax1.set_ylim(0, 105)
    ...
    x_pw = subject_stats["prewalk_rate"].values * 100
    y_esc = subject_stats["escape_rate"].values * 100
    ax2.scatter(
        x_pw,
        y_esc,
        color="#3C5488",
    ...
    if len(x_pw) > 2 and np.std(x_pw) > 1e-6 and np.std(y_esc) > 1e-6:
        r, p = pearsonr(x_pw, y_esc)
        m, b = np.polyfit(x_pw, y_esc, 1)
        x_line = np.linspace(0, max(float(x_pw.max()), 10.0), 100)
        ax2.plot(
            x_line,
            m * x_line + b,
            color="black",
        ...
        ax2.text(
            0.05,
            0.90,
            f"Pearson r = {r:.2f} ({sig_str})\np = {p:.3g}",
            transform=ax2.transAxes,
            fontsize=8,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="white", edgecolor="0.8"),
        )

    ax2.set_xlabel("PreWalk Rate (%)", fontsize=9)
    ax2.set_ylabel("Escape Rate (%)", fontsize=9)
    ax2.set_title("B. PreWalk vs. Escape Coupling", fontweight="bold", fontsize=10)
