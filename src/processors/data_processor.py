...
class DataProcessor:
    ...
    def _discover_csv_pairs(
        ...
        kin_files = sorted(input_dir.glob("*kinematics*.csv"))
    ...
    def _load_kinematics(self, filepath: Path) -> pd.DataFrame:
        """Load and validate a kinematics CSV."""
    ...
    def _load_events(self, filepath: Path) -> pd.DataFrame:
        """Load and validate an events CSV."""
        df = pd.read_csv(filepath)
        required = {"sys_time", "event_type", "state"}
        if not required.issubset(df.columns):
            missing = required - set(df.columns)
            raise ValueError(f"Missing columns in {filepath.name}: {missing}")
        return (
            df[["sys_time", "event_type", "state"]]
            .dropna()
            .sort_values("sys_time")
            .reset_index(drop=True)
        )
    ...
    def _compute_psth_for_event_type(
        ...
        time_axis_psth = np.linspace(pre_s, post_s, n_total)
