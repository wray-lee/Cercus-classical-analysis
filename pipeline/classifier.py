...
def classify_trial(trial: pd.DataFrame) -> dict[str, str | float]:
    ...
    1. **No-burst intercept** — no valid escape burst detected (``v_max`` ≤ 50
       ...
       but ``latency_ms`` forced to NaN.

    2. **PreWalk intercept** — a valid burst exists **and** pre-escape
       spontaneous activity exceeds ``PREWALK_THRESHOLD`` in the 1-s window
       before ``latency_ms``.  Unified anchor for ALL trial types.

    3. **Escape** — a valid burst exists **and** no pre-walk activity.
    ...
    # ── 3. Priority 2 — PreWalk: walking detected in the 1 s before escape ──
    # Unified anchor at latency_ms for ALL trial types (wind, bimodal,
    # baseline_visual).  This avoids the old stimulus-onset anchor which
    # missed walking that started between TTC and escape.
    prewalk_anchor = latency_ms
    ...
    # (ponytail: removed the old NoResponse fallback for baseline ≥ 10;
    # a valid burst with no walking is always Escape.)
